...
def _run(path: Path) -> None:
    ...
    rows = [
        adapter.run(F2CRequest(item, "POLYGON EMPTY", 1, 1, 1, 1, 0, "EPSG:0", "boustrophedon"))
...
def _compare(ours_csv: Path, reference_csv: Path) -> None:
    ...
    with ours_csv.open(newline="", encoding="utf-8") as handle:
        ours_rows = [
            ours.run(
                F2CRequest(
                    row["request_id"], "POLYGON EMPTY", 1, 1, 1, 1, 0, "EPSG:0", "boustrophedon"
                )
            )
