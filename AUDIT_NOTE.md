...
## 留痕：robust_union 逐对路径改平衡树归约（性能修复，结果数学同一）

**触发**：v5 双机具重跑实测 ee_field_37 的 RS 配置单次 153 s，剖析定位 150.7 s
在校验器 `outside_area` 的 `robust_union` **逐对回退**：no_headland 配置路径 574 段，
左结合 `reduce(l.union(r))` 每步都把累积大几何与小片做并集，O(n²)。同一调用里
`union_all`（网格路径）只花 2.2 s。
...
## Block D 前批判性自查（2026-08-22，评审五项落地后的自查轮）

四项核验，两项干净两项真问题，全部留痕：

**干净 1**：`derived_status` 接管聚合层后对 v7 是统计 no-op——重算
summarize_pareto 与 manifest 逐项比对（n_instances/zero_ok/degenerate/
front_median/pool_hash/逐实例有效池 3 776 键）全部相等。分歧行
（not_applicable→outside_area）在两种口径下都不是 ok，不改任何池统计。
v7 作为论文数据的地位不受本轮代码改动影响。

**干净 2**：调试探针田核查——ee_field_105（闭合误差测量）、ee_field_103
（前沿图实例）、nl_field_7（颈缩两片）、f2b_004/006/011（O2 对账）均训练侧。

### 真问题 1：两个调试探针田在留出集里（披露，不可也不得重封）

ee_field_37（v5 性能剖析：robust_union 150.7 s 定位）与 ee_field_117
（§4.1 other 分桶样例查看）**在 70 块留出集内**。查看的性质是计时与
失败分类，**没有看任何目标值/前沿统计**，且据此改的代码（树归约、
对偶加速、ULP 容差）作用于全语料而非偏好这两块地——无逐田调参。
但封存主张是「实验开始后留出集没有被**动过**」，严格说这两块的运行行
被检视过：**披露即义务，影响评估为低但非零**。处理（修正案 03 钉死）：
H3 留出集评估报全 70 块与剔除这 2 块的双结果，双轨一致才可下结论；
**不得重封**——看过结果之后换留出集正是封存语义禁止的事。

### 真问题 2：双机具设计的统计单位漂移（伪重复，Block D 最大统计风险）

修正案 02 定死「独立单位 = n=235 地块」，但它早于双机具一天。v7 实例
= 地块×5 角×2 行距×2 机具 = 4 700，若 H1/H2 把实例当独立单位，
显著性被同地块相关性虚增近 √20 倍——这是伪重复，必须在地块层聚合。
修正案 03（本轮新增）钉死：H1/H2/H3 独立单位恒为地块；H1 估计量 =
地块内（有前沿的实例上）前沿大小的中位数再对地块做检验；零 ok 实例
不进主估计量但零 ok 地块份额（42/235）与敏感性口径（零 ok 计 0）并报；
分析池 = corpus_13（pool_hash aa0e4aaa…）。**已见统计量披露**：基建
验证期已见过实例层前沿中位 v4 2.0 → v7 3.0（manifest 必然携带的
描述统计），地块层 H1 估计量尚未计算——在修正案封存之前不算。

### 缺口：CV 折指派未落盘（Block D 首件基建）

manifest 只记 cv_folds=10 的数字；折指派代码与落盘文件都不存在。
H3 推荐器训练前必须：折按 field_id 分组（C-R1 同键）、种子写死、
指派表落盘并进哈希链——折的构造发生在任何训练之前，不许训练后再补。
...
## 第三次转折发起（2026-08-24）：核验、修正案 04 与生成式研究线章程

外部工程验收（main=76e4a92）与五线文献调研两份输入，批判性核验后落地：

- **核验属实的**：546/冻结件/双自检本地复跑一致；F2C recorded 自检修复
  （有效教训：单测绿 ≠ 公开入口可运行）；O2 路线身份对齐的 transit
  −0.124%（但此为既有结论「配对后 −0.14%」的独立再确认，非新知识；
  旧开放项「转移分解未闭合」据此关闭）。
- **实证判死 H2 修正案 03 口径**：实例特征 row_angle_vs_principal =
  设计偏移 + 主轴估计噪声（每档跨田极差 ≤0.03 rad，档间步长 0.39）；
  田级中位全挤 π/4±0.03——跨田 Spearman 测的是仪器噪声。修正案 04 立：
  H2 改田内重复测量（每田 5 偏移档、档内中位、田内 ρ、Wilcoxon 对 0），
  H3 首要端点改偏好条件加权 Tchebycheff 悔值（26 固定单纯形偏好），
  原 HV 比值降为次要。**均在任何检验执行前封存。**
- **转折点**（docs/TURNING_POINT.md）：从选择自然算法到证据纪律下生成
  算法——LLM 进化循环 × 本基准作评估器。联网抽查确认该交叉为空白
  （EoH/EoH-S/G-LNS/LLM4AD 簇 ∩ 农业 CPP 簇 = 空；定稿前须复查）。
  序列锁定：债务 → Block D（选择层，产出自然池互补有限的量化动机）→
  Study-002 预注册（生成式）。非目标：不再大重构、不捆绑能耗/多机。

## D0.4 执行规范封口（修正案 05，最终修正案，2026-08-24）

外部复核指出修正案 04 的四处可执行性缺口，逐一核实**全部属实**并封死：

1. **零权重 P0**：contracts/preference.py 原 `weight > 0` 使单纯形顶点
   （1,0,0）不可表达——修正为 `≥ 0` + 权重和 > 0，网格点三维全部显式
   声明，顶点回归测试钉住。
2. **偏好网格坐标级冻结**：修正案 04 的「26 点」无唯一定义（棱点歧义 +
   内部 14 点无数学定义），废除；PREFERENCE_GRID_V1 = 3 顶点 + 9 棱点
   （t∈{1/4,1/2,3/4}）+ 10 内部格点（(i,j,k)/6），共 22 点，坐标列于
   代码与 evidence/preference_grid_v1.json，哈希 0ed03d3f… 进协议身份。
3. **H3 基线去 oracle**：有效池抽样改为静态可适用池上的**精确期望**
   （无 RNG）；三层基线（random_applicable 主 / random_nominal /
   SBS-on-training 次）；oracle 只作悔值归一化地板不进抽取集合。
4. **H3 田级聚合**：D_f = L_f^rec − 0.5·L_f^rand，符号翻转置换 10^4
   （seed 20260822），伪重复封死；不可行推荐罚则 R_max+1 写死。
5. **H2 常数响应**：田内前沿恒定时 ρ := 0（计入检验不剔除——剔除会
   系统性偏向正结论）；档位计数与全 5 档敏感性并报。
6. **措辞纪律**：行向失配是 scenario factor，对外只说「受控田内处理
   效应」，不作真实生产因果外推。
...
## Study-001 结案（D7/H3 封存，ledger index=6，2026-08-24）

**H3 结果：未获支持。** 留出集一次性消费（70 田，12 块零 ok 单列）：
推荐器 mean 悔值 0.3437 vs random_applicable 精确期望 0.5699，其半
0.285——推荐器未低于一半，**D 均值 +0.0587，置换 p=0.821，预注册失效
判据 1 触发**。判据 2（不可行率 0.244 < 随机 0.331）未触发。双轨 70/68
完全一致：两块探针田本属零 ok 田，剔除不改结果。描述性观察（不作辩护
依据）：70.7% 的田个体上 D<0，重尾（不可行罚值）拖正均值——与 D4 CV
中的 SBS 竞争力警告同源。

**Holm 家族终表**（α=0.01，三条按 p 升序）：

| 假设 | 原 p | Holm 调整 | 结论 |
|---|---:|---:|---|
| H1 前沿>1 | 2.39e-30 | 7.18e-30 | **支持** |
| H2 田内 ρ>0 | 1.20e-12 | 2.40e-12 | **支持** |
| H3 悔值<半随机 | 8.21e-01 | 0.821 | **不支持（失效判据触发）** |

**执行事故留痕（诚实记录）**：封存前两次运行中止于身份守门——
analyze_h3.py 的 EXPECTED 哈希表存在转写笔误（第一次 33d…，修复时又
误写 33a…，正值为 33b…）。两次中止均发生在评估完成之后、任何写盘
之前：无结果输出、无泄漏、账本未动；第三次以从已封存 h1_result.json
逐字节提取的正值通过守门并完成封存。守门连续两次抓住实现方错误，
这是它存在意义的现场演示。

**科学解读（留给论文，不改动任何注册口径）**：H1/H2 成立而 H3 失败
的组合正是生成式研究线的动机实证——自然 13 配置池存在真实权衡结构
（H1）且由田内失配驱动（H2），但偏好条件选择器在留出田上无法把悔值
压到随机基线一半以下（与 D4 中 SBS 的竞争力一致）。选择的空间有限，
生成的价值由此立论。Study-001 关闭，后续按 TURNING_POINT 序列进入
Study-002 预注册。
