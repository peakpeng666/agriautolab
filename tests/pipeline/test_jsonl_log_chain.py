...
payload invalidates every later entry. This matters because `replace_unless_sealed`
...
the public `build_entry()`) must be caught by `verify_entries` before the sealed-
...
def test_rewritten_entry_with_recomputed_own_hash_is_rejected(tmp_path: Path):
    ...
    entries[1] = jsonl_log.build_entry(1, forged)  # attacker recomputes only this entry's own digest
...
def test_sealed_guard_cannot_be_bypassed_by_log_rewrite(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="sealed evidence"):
        jsonl_log.replace_unless_sealed(replacement, final, log, "sealed_result", "file_sha256")
    ...
    entries[1] = jsonl_log.build_entry(1, forged_payload)  # re-key the recorded seal, leave the tail alone
    log.write_text(
...
def test_genesis_entry_has_no_predecessor():
    genesis = jsonl_log.build_entry(0, {"event": "cv_assignment_sealed"})
...
def test_editing_an_earlier_payload_invalidates_every_later_entry(tmp_path: Path):
    ...
    entries[1] = jsonl_log.build_entry(1, dict(entries[1]["payload"], nominal_size=99), entries[0]["entry_hash"])
    with pytest.raises(ValueError, match="mismatch at index=2"):
        jsonl_log.verify_entries(tuple(entries))


def test_entry_after_derives_index_and_predecessor():
    genesis = jsonl_log.build_next_entry((), {"event": "cv_assignment_sealed"})
    assert genesis["index"] == 0
    assert genesis["prev_hash"] is None
    follower = jsonl_log.build_next_entry((genesis,), {"artifact": "pool_census"})
