"""HoldoutPartition：封存一次、对账拒不一致。"""

import pytest

from agriautolab.selection.holdout_partition import HoldoutPartition


def test_seal_once_then_verify_accepts_identical_holdout() -> None:
    partition = HoldoutPartition()
    seal = partition.seal_holdout(("p3", "p1", "p2"), seed=20260821)
    assert seal.problem_ids == ("p1", "p2", "p3")   # 排序封存，顺序不是身份的一部分
    partition.verify(("p2", "p1", "p3"), seed=20260821)


def test_double_sealing_is_rejected() -> None:
    partition = HoldoutPartition()
    partition.seal_holdout(("a",), seed=1)
    with pytest.raises(RuntimeError, match="重新封存"):
        partition.seal_holdout(("b",), seed=2)


def test_verify_rejects_changed_holdout_and_unsealed_partition() -> None:
    with pytest.raises(ValueError, match="尚未封存"):
        HoldoutPartition().verify(("a",), seed=1)
    partition = HoldoutPartition()
    partition.seal_holdout(("a", "b"), seed=1)
    with pytest.raises(ValueError, match="不一致"):
        partition.verify(("a", "c"), seed=1)
    with pytest.raises(ValueError, match="不一致"):
        partition.verify(("a", "b"), seed=2)


# ---- 封存需按 field_id 分组 ----------------------------------------

def test_all_instances_of_one_field_share_the_same_in_holdout_flag() -> None:
    """同一 field_id 的全部实例，in_holdout 需相同。
...
def test_sealing_field_level_holdout_round_trips_through_the_partition() -> None:
    """封存的是 field_id，不是实例 id——对账也需在同一粒度上。"""
    ...
    holdout = field_level_holdout(fields, fraction=0.3, seed=20260821)
    partition = HoldoutPartition()
    seal = partition.seal_holdout(holdout, seed=20260821)
    assert seal.problem_ids == holdout
    partition.verify(field_level_holdout(fields, fraction=0.3, seed=20260821), seed=20260821)
    with pytest.raises(ValueError):
        partition.verify(holdout[:-1], seed=20260821)
