...
版本号遵循语义化版本。历史研究的完整过程留痕见 `AUDIT_NOTE.md`（副本位于
`study-001-frozen` tag）。
...
- **TSPLIB / CVRPLIB 标准实例接入**：新增 `datasets/tsplib.py`——
  ...
  `optimality_gap`，与几何距离明确分开。`nint` 按规范实现为
...
- **组合优化方法学验证层**：将 TSP/CVRP 作为农业 CPP 自动算法设计前的正式
  ...
  非数值/NaN/Inf 以及任意自定义 `float(score)` 转换异常，并保留异常链。
- **release / CI hygiene**：主线版本改为 `0.5.1.dev0`；CI 权限收敛为只读，
  ...
  GitHub Actions 供应链。
- **anytime 性能轨迹**：`EvolutionRecord` 加 `evaluations_used`
...
- **route_order 槽位**：新增第二候选槽位
  ...
  后访问序会变——这是正确行为，不是 bug。
- **LLM provenance 入账与重放校验**：`proposer.CompletionResult`
...
- 第一轮完整研究结案：三条确证检验全部封存（Pareto 前沿支持 / 行角效应支持 /
  推荐器评估不支持），Holm 族级调整终表与封存前两次身份守门中止事故的
  contemporaneous 留痕见 `study-001-frozen` tag。
- 三层池普查（nominal / static-applicable / observed-OK 契约 + 逐实例包含校验）、
  `scripts/pool_census.py` 与基准结果账本 index=1。
- 修正案 04/05 的统计设计落地（田内重复测量、偏好条件 Tchebycheff 悔值、
  常数响应记 0、非 oracle 精确期望基线），`examples/corpus/` 补齐。
- field-grouped CV 身份冻结：235-field universe − 70-field holdout = 165
  training fields，seed 20260822、10 折；完整折表与 assignment/spec hash 落盘。
- 工程成熟化：GitHub Actions CI（ruff + 双 Python 矩阵 pytest）、ruff 接入、
  README 重写、CONTRIBUTING、PR/Issue 模板、CHANGELOG。
- 命名/注释对齐：轮次标签从生产源码改写为语义描述；迭代痕迹清零
  （轮次标签、复核编号、任务号、裸章节号移出生产代码）。

## [0.4.0] — 2026-08-23

### canonical 命名层（PR #1）

- `docs/NAMING.md`：总纲——证据身份（wire ID）稳定不变，规范名只在 API 层。
...
- 验收：pool_hash/协议哈希/冻结件字节/parquet 契约对数据集全量重放一致。

## [0.3.0] — 2026-08-23

### 终语料与复核落地

- 终跑（61,100 行四桶封闭，**other=0、crash=0**）；66 崩溃行去向逐一对账。
...
- 溯源件：61,101 条账本逐条复算验证全过。
- 修正案 03：统计单位钉地块层、留出集探针披露与双轨报告。

## [0.2.0] — 2026-08-22

### WSL 迁移、一键安装与双机具

- 一键安装五件套（apt/F2C 编译/Python/校验/锁文件）+ 净室实测九项自校验全过。
- Python 下限 3.11→3.10（WSL 迁移轮）；跨平台字节冻结（.gitattributes LF）。
- 状态分类归零：具名状态映射（封闭词典 + 未知原因响亮失败）。
- manifest 取聚合器（零 ok 实例不再静默消失）。
- 双机具 + `vehicles_hash` 协议强制核对。
- checkpoint 跑完 gzip（mtime=0 字节确定）。
