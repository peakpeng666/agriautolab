...
[![tests](https://img.shields.io/badge/tests-560%20passed-brightgreen)](tests/)
...
[docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)。L4 选择层（Block D）已开工：
D0.4 confirmatory protocol 封口、D1 field-grouped CV 身份冻结完成，下一步 D2
拒绝/有效池普查；**推荐器尚未训练，H1/H2/H3 尚未正式执行**。
...
pytest -q          # 560 passed / 30 skipped
```
...
| Pareto 前沿 | manifest 实例层中位 3.0；正式 H1 尚未执行地块层检验 |
...
执行规范封口。独立统计单位为 field；留出集 70/235 地块在分析前密封，H3
只在最终评估阶段一次性打开并按 70/68 双轨报告。D1 训练折在任何推荐器训练
