...
六个字里只有 LRL 错，而五个手工样例一个都没命中它——正演闭合需随机、需大量。
本模块的 LRL 按几何推导：
...
def dubins_words(start: Pose2D, goal: Pose2D, radius: float) -> tuple[DubinsWord, ...]:
    ...
    if radius <= 0.0:
        raise KinematicModelError(f"Dubins radius must be greater than 0, got {radius!r}")
...
def dubins_length(p0: tuple[float, float, float], p1: tuple[float, float, float], radius: float) -> float:
    """(x,y,theta) -> (x,y,theta) 的最短 Dubins 路长。radius 需 > 0。
