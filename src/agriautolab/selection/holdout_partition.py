...
class HoldoutPartitionRecord(BaseModel):
...
class HoldoutPartition:
    ...
    def __init__(self) -> None:
        self._seal: HoldoutPartitionRecord | None = None

    @property
    def seal(self) -> HoldoutPartitionRecord | None:
        return self._seal

    def _hash(self, problem_ids: tuple[str, ...], seed: int) -> str:
        return content_hash({"problem_ids": sorted(problem_ids), "seed": seed})

    def seal_holdout(self, problem_ids: tuple[str, ...], *, seed: int) -> HoldoutPartitionRecord:
        if self._seal is not None:
            raise RuntimeError("留出集已封存：重新封存等于换留出集，拒绝")
        if not problem_ids:
            raise ValueError("留出集不能为空")
        self._seal = HoldoutPartitionRecord(
...
def field_level_holdout(
    ...
    **需按地块分组，不能按实例分组**（与 CV 折分组同一条纪律）：
    ...
    if not 0.0 < fraction < 1.0:
        raise ValueError(f"holdout fraction must be within (0,1), got {fraction!r}")
...
def instance_in_holdout(field_id: str, holdout_field_ids: Iterable[str]) -> bool:
    ...
    这是「同一 field_id 的全部实例 in_holdout 需相同」这条不变量的唯一实现点：
